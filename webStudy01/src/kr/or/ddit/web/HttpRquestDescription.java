...
/**
 * Http 프로토클의 request packaging 규칙
 * Request Line : URL, Http Method, Protocol/version
 * 		Http Mehtod(Request Method) : 현재 요청의 목적/의도/수단/방식
 * 		GET		: 조회(R)
 ...
 * 		String[] getParameterValues(name) : 동일한 파라미터 명으로 여러개의 값이 전송.
 * 	    Enumeration getParameterNames()
 * 		** Map<String,String[]> getParameterMap()
 * 		** 요청 파라미터에 특수문자가 포함된 경우,
 * 		   IETF에서 정의한 RFC2396 규약에 따라
 * 	 	    모든 특수문자는 URLEncoding(Parcent Encoding)방식으로 인코딩되어 전송.
 * 		    인코딩된 특수문자 확보
...
public class HttpRquestDescription extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(request.getRemotePort()); 	//클라이언트의 포트넘버 80
